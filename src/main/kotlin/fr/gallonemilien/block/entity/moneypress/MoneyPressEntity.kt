...
class MoneyPressEntity(pos: BlockPos, state: BlockState) :
    ...
    override fun writeNbt(nbt: NbtCompound?, registryLookup: RegistryWrapper.WrapperLookup?) {
        ...
        Inventories.writeNbt(nbt, moneyPressInventory.inventory, registryLookup)
    ...
    fun tick(world: World, pos: BlockPos, state: BlockState) {
        ...
        if (moneyPressInventory.isOutputSlotEmptyOrReceivable() && MoneyPressRecipe.matches(moneyPressInventory)) {
            if(!isWorking)
                isWorking = true
            ++progress
            ...
            if (progress == maxProgress) {
                ...
                moneyPressInventory.removeInputItem()
                moneyPressInventory.addOutputItem(MoneyPressRecipe.getResult())
            }

            if(progress >= waitForReset) {
                progress = 0
            }
        } else {
            if (progress != 0) {
